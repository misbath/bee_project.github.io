...
```
>>>>>>> 577258fd9b1fc9fa12e0acd2c93aaeec368444b1

```{r, correlation pesticides}

corr_2004 = read_csv("./data/top_pesticides.csv") %>% 
  group_by(year, compound) %>% 
  summarise(tot_pest_high = sum(epest_high_kg)) %>% 
  filter(year == 2004) %>% 
  select(compound, tot_pest_high) %>%
  pivot_wider(
    names_from = "compound",
    values_from = "tot_pest_high",
  )

cor(corr_2004, use = "everything", method = c("pearson"))
