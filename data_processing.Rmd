...
cor1 = cor(corr_al_data, use = "everything", method = "pearson")
...
cor2 = cor(corr_ak_data, use = "everything", method = "pearson")
...
cor3 = cor(corr_as_data, use = "everything", method = "pearson")
...
cor4 = cor(corr_ar_data, use = "everything", method = "pearson")
...
cor5 = cor(corr_ca_data, use = "everything", method = "pearson")

state_correlations = bind_rows(cor1, cor2, cor3, cor4, cor5)
```
...
correlation_function = function(x) {
  corr_data = 
  merged_state_data %>%
  filter(state_fips.x == x) %>%
  group_by(year.x) %>%
  summarize(pesticide = sum(high), honey_producing_colonies = ((sum(honey_producing_colonies))/5)) %>%
  select(-year.x)
  
cor(corr_data, use = "everything", method = "pearson")
  
}

correlations = map_dfr(.x = merged_state_data, ~correlation_function(.x))
